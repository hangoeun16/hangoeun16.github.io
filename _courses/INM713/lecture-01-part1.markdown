...
Note: This is my own interpretation based on self-study based on the lecture slides. While I've tried to be accurate, some of my commentary and understanding may be imperfect. 

#### Data Graph
Data graph is the foundation of any knowledge graph. Its key property includes:
1. intuitive abstractions to capture entities and their relationship
2. do not require schema to start adding data $\to$ flexibility to assemble data from multiple sources
3. enable queires over paths of arbitrary length
4. potential to correspond with logical unary and binary predicates.
