...
date: 2026-01-01
tags: [general, project, GraphRAG, chatbot]
description: >
    A technical reflection on building KinGraph, an LLM-powered knowledge graph chatbot — design decisions, attempted approaches, and lessons learned.
author: Summer Han
toc:
  sidebar: left
...
Initially, to resolve this issue, I thought about creating a chatbot that shows a family tree along with the chat UI, so that the user can check if the chatbot is accurately reflecting the family relationships.
...
### What does KinGraph Chatbot do?
- Chat with GPT-4o (via LangChain) in a friendly conversational style
- Store family relationships in a knowledge graph (NetworkX)
- Extract people, family relationships, and personal attributes from messages
- Detect conflicts with previously stored information
- Visualise a non-hierarchical family tree as an interactive graph
...
3. **Alternative UI layouts:**
   ...
   - Multiple pages: First page for the chatbot, second page for the family tree. This proved inconvenient for users, as they had to move back and forth between pages.
...
### Remaining Issues
- Random shutdown issue: When a session has been open for more than 7 to 8 minutes, the programme partially breaks (it appears as though pixels are broken at the edge of the chat UI) and performance decreases. While this could be a problem with my laptop or some other aspect of the code construction, I plan to investigate further.
- Placeholder and conflict detection issue: While it works most of the time, the placeholder sometimes fails when used alongside conflict detection. Since this occurs randomly, I would like to investigate further to pinpoint the specific condition where the error occurs.
